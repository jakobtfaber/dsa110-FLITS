...
**Superseded (2026-07-27 note):** the "future ADR" arrived — [ADR-0006](docs/adr/0006-beta-coherent-scattering-comodel.md) makes β the sampled parameter (α derived via α = 2β/(β−2); free-α sampling retired), and [ADR-0008](docs/adr/0008-retrust-validation-contract.md) supersedes the ADR-0005 citable-α roster and every campaign-era PASS/MARGINAL/FAIL verdict *for quoting* until fits are re-run under the retrust contract. The current manuscript carries no active `tab:alpha`/`tab:beta` and quotes no τ/α/β value pending the production refit (`sections/results.tex`). The two-track vocabulary above remains for reading historical artifacts.
...
**Superseded (2026-07-27 note):** the 49-row/`scratch/codetection/` framing is historical — the scratch chain is no longer present in the checkout (regenerators only; registry is SSOT), and the manuscript now states the **extended census** (provenance tag `v4-extension-2026-07-15`): 38 catalog rows → 31 physical systems after deduplication; 11 confirmed foreground (9 halos + 2 clusters) + 15 inconclusive retained, 5 refuted. The live registry carries **52 rows: 29 confirmed / 16 inconclusive / 7 refuted** (pre-dedup, including outside-aperture clusters). The 29/7/13 prose counts below anchor the *original* validation set, not current manuscript numbers.
...
- **"Scintillation measured"** — SUPERSEDED 2026-07-27: the "only casey/freya/wilhelm attempted, 9 not-yet-attempted" state is historical. The finalized window-tuning campaign (2026-07-17, `analysis/window-tuning-campaign-2026-07-17/results/`) evaluated the sample and yields one qualified high-resolution CHIME record (`chromatica_hi`) plus 23 diagnostic-only records (`analysis/chime-scintillation/README.md`); the manuscript reports one qualified CHIME sightline and one DSA measurement (`sections/results.tex`). Owner ruling 2026-07-26: the entire scintillation campaign is **first-pass evidence, not final** — manuscript numbers come from the chartered interactive re-do from raw data (scint-redo-01, analysis repo). Still do not conflate "no qualified measurement" with "unsuitable".
