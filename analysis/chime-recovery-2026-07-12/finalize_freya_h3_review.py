#!/usr/bin/env python3
"""Bind the completed visual review into the Freya H3 validation verdict."""
...
RESULT_DIR = ROOT / "analysis/chime-recovery-2026-07-12/results/h3"
...
def main() -> int:
    ...
    validation["transfer_qualification_status"] = "fail" if failed else "pass"
    validation["science_status"] = "diagnostic_only"
    validation["campaign_status"] = "documented_fail" if failed else "pass"
    validation_path.write_text(json.dumps(validation, indent=2, sort_keys=True) + "\n")
    verdict = {
        "campaign_status": validation["campaign_status"],
        "failed_checks": failed,
        "on_pulse_application_performed": validation["on_pulse_application_performed"],
        "science_status": validation["science_status"],
    }
    print(json.dumps(verdict, indent=2, sort_keys=True))
    return 0 if validation["campaign_status"] == "pass" else 2
