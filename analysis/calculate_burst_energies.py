...
from analysis.flux_cal import dsa_band_fluence_jy_ms_hz  # noqa: E402
...
def compute(scales: dict[str, float | str | None] | None = None) -> list[dict]:
    ...
    for nick in sorted(fits):
        ...
        I_C_jy, cal_C = _band_jy(s_C, I_C, nick, None)  # CHIME fluxcal: Phase 6
        I_D_jy, cal_D = _band_jy(s_D, I_D, nick, dsa_band_fluence_jy_ms_hz)
...
def markdown_table(rows: list[dict]) -> str:
    if rows and rows[0]["calibrated"]:
        head = (
            ...
            "E_CHIME (erg) | E_DSA (erg) | E_iso (erg) | E_iso no-(1+z) (erg) |"
        )
        sep = "|" + "|".join(["---"] * 9) + "|"
        lines = [head, sep]
        for r in rows:
            lines.append(
                ...
                f"{r['E_iso_erg']:.3e} | {r['E_iso_erg_no_kcorr']:.3e} |"
            )
        return "\n".join(lines)
...
$E_{{\mathrm{{iso}}}}$ includes the $(1+z)$ k-correction.}}
