...
ALL = "casey chromatica freya hamilton isha johndoeII mahi oran phineas whitney wilhelm zach".split()
...
def band_metrics(m, p, gain):
    ...
    ns = np.asarray(m.noise_std, float); ns = ns[:, None] if ns.ndim == 1 else ns
...
def classify(b, P, mC_chi, mD_chi, mC_lag, mD_lag):
    al, alo, ahi = P["alpha"]["median"], P["alpha"]["lower"], P["alpha"]["upper"]
    zc, zd = P["zeta_C"]["median"], P["zeta_D"]["median"]
    flags = []
    if alo <= A_LO + EDGE: flags.append("aRAIL_LO")
    if ahi >= A_HI - EDGE: flags.append("aRAIL_HI")
    if max(zc, zd) > ZETA_MAX: flags.append(f"zeta_hi({max(zc,zd):.0f})")
    if mC_lag > LAG1_MAX: flags.append(f"CHIME_temporal({mC_lag:.2f})")
    if mD_lag > LAG1_MAX: flags.append(f"DSA_temporal({mD_lag:.2f})")
    if max(mC_chi, mD_chi) > CHI2_MAX and not any("temporal" in f for f in flags):
        flags.append(f"chi2_hi({max(mC_chi,mD_chi):.1f})")
    verdict = "MEASUREMENT" if not flags else (
        "EXCLUDE" if any(k in " ".join(flags) for k in ("temporal", "RAIL", "zeta_hi"))
        else "MARGINAL")
...
def main():
    ...
    for b in bursts:
        ...
        if not os.path.exists(jf):
            rows.append((b, "NO_FIT", [], None)); continue
        d = json.load(open(jf)); P = d["percentiles"]
        gain = bool(d.get("marginalize_gain", False))
        p = {k: v["median"] for k, v in P.items()}
        tau, al = p["tau_1ghz"], p["alpha"]
        mk = lambda s: FRBParams(c0=1.0 if gain else p[f"c0_{s}"], t0=p[f"t0_{s}"],
                                 gamma=0.0 if gain else p[f"gamma_{s}"], zeta=p[f"zeta_{s}"],
        ...
        cC, sC, lC, modC, rC = band_metrics(mC, mk("C"), gain)
        cD, sD, lD, modD, rD = band_metrics(mD, mk("D"), gain)
        verdict, flags = classify(b, P, cC, cD, lC, lD)
        rows.append((b, verdict, flags, dict(al=al, alo=P["alpha"]["lower"], ahi=P["alpha"]["upper"],
                                             tau=tau, cC=cC, cD=cD, lC=lC, lD=lD,
                                             zC=p["zeta_C"], zD=p["zeta_D"], lnZ=d.get("log_evidence", 0))))
        # resid map
        fig, ax = plt.subplots(2, 3, figsize=(13, 7))
        for row, m, mod, resid, nm, ch, lg in [
            (ax[0], mC, modC, rC, "CHIME", cC, lC), (ax[1], mD, modD, rD, "DSA", cD, lD)]:
            ext = [m.time.min(), m.time.max(), m.freq.min(), m.freq.max()]
            vmx = np.nanpercentile(np.abs(m.data), 99)
            row[0].imshow(m.data, aspect="auto", origin="lower", extent=ext, cmap="magma", vmin=0, vmax=vmx)
            row[1].imshow(mod, aspect="auto", origin="lower", extent=ext, cmap="magma", vmin=0, vmax=vmx)
            row[2].imshow(resid, aspect="auto", origin="lower", extent=ext, cmap="coolwarm", vmin=-4, vmax=4)
            row[0].set_ylabel(f"{nm}\nf (GHz)")
            row[1].set_title(f"model"); row[2].set_title(f"resid chi2={ch:.2f} lag1={lg:.2f}")
        ax[0][0].set_title(f"{b} data")
        fig.suptitle(f"{b}: {verdict}  alpha={al:.2f} tau1={tau:.3f}ms  {' '.join(flags)}", fontsize=11)
        fig.tight_layout(); fig.savefig(f"{out}/{b}_resid_map.png", dpi=110, bbox_inches="tight"); plt.close(fig)

    print(f"{'burst':12s} {'verdict':12s} {'alpha[lo,hi]':18s} {'tau1':7s} "
          f"{'chiC':5s} {'chiD':5s} {'lagC':5s} {'lagD':5s} flags")
    print("-" * 105)
    nmeas = 0
    for b, v, flags, m in rows:
        if m is None:
            print(f"{b:12s} {v:12s}"); continue
        a = f"{m['al']:.2f}[{m['alo']:.2f},{m['ahi']:.2f}]"
        print(f"{b:12s} {v:12s} {a:18s} {m['tau']:7.3f} {m['cC']:5.2f} {m['cD']:5.2f} "
              f"{m['lC']:5.2f} {m['lD']:5.2f} {','.join(flags)}")
        if v == "MEASUREMENT": nmeas += 1
