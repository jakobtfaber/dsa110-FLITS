...
hdr = (f"{'burst':12s} {'json':5s} {'npz':4s} {'alpha (p16,p84)':20s} "
...
for b in BURSTS:
    ...
    print(f"{b:12s} {'yes':5s} {('yes' if hass else 'NO'):4s} {astr:20s} "
          f"{ddmc:+8.1f} {ddmd:+8.1f} {zc:6.2f} {zd:6.2f} {lnz:10.1f} {','.join(flags)}")
    if flags:
        nflag += 1

print()
print(f"summary: {len(BURSTS)} bursts | {nfail} missing/failed | {nflag} flagged "
      f"(alpha rails / dDM degeneracy / no samples)")
print("note: aRAIL_LO = alpha posterior pressing the 1.0 prior floor -> floor may be "
