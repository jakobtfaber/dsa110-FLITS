...
def main():
    ...
    c0g, t0g, gmg = ip("c0_C", "c0_D"), ip("t0_C", "t0_D"), ip("gamma_C", "gamma_D")
    ztg, ddg = ip("zeta_C", "zeta_D"), ip("delta_dm_C", "delta_dm_D")
    yG = np.zeros((GAPNCH, tg.size))
    for i, fg in enumerate(gfreq):
        m1 = FRBModel(time=tg, freq=np.array([fg]), data=np.zeros((1, tg.size)),
                      dm_init=mD.dm_init)
        pg = FRBParams(c0=c0g[i], t0=t0g[i], gamma=gmg[i], zeta=ztg[i],
                       tau_1ghz=tau, alpha=al, delta_dm=ddg[i])
        yG[i] = m1(pg, "M3")[0]
    ...
    for ax, W, title in [
        ...
        (axm, mdl_all, f"{b}  JOINT MODEL  (row-normalized; gap = extrapolation; alpha={al:.2f}, tau1GHz={tau:.2f} ms)"),
    ]:
        ax.pcolormesh(tg, freq_all, ma.masked_invalid(W), cmap=cmap,
    ...
    axd.text(tg.mean(), 0.5 * (fclo + fchi), "inter-band gap\n(no data)",
             ha="center", va="center", color="0.45", fontsize=10)
    axm.text(tg.min() + 0.04 * (tg.max() - tg.min()), 0.5 * (fclo + fchi),
             "gap: tau(nu)+dispersion\nextrapolation only\n(intrinsic envelope interp.)",
             ha="left", va="center", color="cyan", fontsize=8, alpha=0.85)
