...
def make_sightline_fig(target_name, z_frb, gal_rows):
    ...
    for idx, g in enumerate(gal_rows):
        ...
        # Galaxy centre
        ax.scatter(g['d_com'], g['impact'], color=c, edgecolors='white',
                   s=100, zorder=5, linewidths=1.2)
    ...
    ax.text(d_frb, ax.get_ylim()[1] if ax.get_ylim()[1] > 50 else 50,
            f"FRB z={z_frb:.3f}", fontsize=8, color=ACCENT_RED, ha='right')
    ...
    legend_elements = [
        ...
        plt.Line2D([0], [0], marker='o', color='none',
                   markerfacecolor=DARK_BLUE, markeredgecolor='white',
                   markersize=10, label='Galaxy Centre'),
        plt.Line2D([0], [0], color=ACCENT_ORANGE, linestyle='--',
                   linewidth=1.5, label='Impact Parameter (b)'),
        Patch(facecolor=LIGHT_BLUE, edgecolor='none', alpha=0.3,
              label='Virial Radius ($R_{200}$)'),
    ]
    ax.legend(handles=legend_elements, loc='upper right', frameon=True,
              facecolor='white', edgecolor=GRID_COLOR, fontsize=8)
    fig.tight_layout()
...
def make_mass_profile_fig(target_name, z_frb, gal_rows):
    ...
    for idx, g in enumerate(gal_rows):
        ...
        m_at_b = nfw_enclosed_mass(g['impact'], g['m_halo'], g['r_vir'],
                                   g['r_s'], g['c'])
        ax.axvline(g['impact'], color=c, linestyle=':', alpha=0.5)
        ax.scatter(g['impact'], m_at_b / 1e11, color=c,
                   edgecolor='white', s=60, zorder=5, linewidths=1.2)
        # R_vir marker
    ...
    ax.set_ylabel(r'Enclosed NFW Mass  $M(<r)$  ($10^{11}\,M_{\odot}$)',
...
def main():
    ...
    for name, ra_str, dec_str, z_frb in TARGETS:
        ...
        if df.empty:
            print(f"  {name}: CSV empty, skipping.")
            continue
