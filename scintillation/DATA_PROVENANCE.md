...
- Authoritative DSA dynamic-spectrum npz products are the CANFAR files at
  `arc:home/jfaber/baseband_morphologies/chime_dsa_codetections/FLITS/scintillation/data/{nick}.npz`.
  The 2026-07-07 pull stages the 11 Mac-missing DSA products via h17
  `$COD/results/dsa_scint_npz/` and then into
  `~/Data/Faber2026/dsa110/scintillation/data/`.
- The DSA ACF pickles formerly at risk under
  `$COD/archive/arc_trash_2026-06/acf_results/` were copied to
  `$COD/results/acf_results/{chromatica,freya,wilhelm}_acf_results.pkl`.
...
> **[from script]** = stated in `$COD/scripts/upchannelize_chime.py`;
...
| johndoeII| 311723353 | 696.506 | 0.696 | [from script] |
| oran     | 224263996 | 396.882 | 74.2 | [from script] |
...
**Outputs — `$COD/upchan_codetections/` (all 12 regenerated 2026-07-06/07):**
**[verified]** each target as `<nick>_chime_upchan.npy` (Stokes-I waterfall,
float32), `<nick>_chime_freq.npy`, and `<nick>_time0_metadata.json`:

| target | CHIME event ID | U | generated log | product status |
|--------|----------------|---|---------------|----------------|
| casey | 362593221 | 16 | `upchannelize_casey_noshift_20260706.log` | local full + hi npz |
| whitney | 215063905 | 16 | `upchannelize_whitney_noshift_20260706.log` | local full + hi npz |
| phineas | 274819243 | 16 | `upchannelize_phineas_noshift_20260706.log` | local full + hi npz |
| mahi | 354049284 | 512 | `upchannelize_mahi_noshift_20260706.log` | local full + hi npz; inspect 700-725 MHz LTE-band RFI suspect |
| isha | 252069198 | 256 | `upchannelize_isha_noshift_20260706.log` | local full + hi npz; upper-bound only |
| freya | 278720455 | 64 | `upchannelize_freya_noshift_20260704.log` | local full + hi npz |
| zach | 210456524 | 64 | `upchannelize_zach_noshift_20260707.log` | local full + hi npz |
| chromatica | 356959136 | 64 | `upchannelize_chromatica_noshift_20260707.log` | local full + hi npz |
| wilhelm | 253635173 | 64 | `upchannelize_wilhelm_noshift_20260707.log` | local full + hi npz |
| oran | 224263996 | 128 | `upchannelize_oran_noshift_20260707.log` | local full + hi npz |
| hamilton | 318353610 | 64 | `upchannelize_hamilton_noshift_20260707.log` | local full + hi npz; single-block product |
| johndoeII | 311723353 | 512 | `upchannelize_johndoeII_noshift_20260707.log` | local full + hi npz; single-block / upper-bound product |

These raw `.npy` products carry **no `times_s`** (spectrum + freq only) and must
be packaged with the metadata-aligned builder. The local packaged products live
under `~/Data/Faber2026/dsa110/scintillation-data/{nick}_chime{,_hi}.npz` and
are symlinked into `~/Data/Faber2026/dsa110/scintillation/data/` for the FLITS
`DynamicSpectrum` config contract.
...
Rescued 2026-07-07 from
`$COD/archive/arc_trash_2026-06/acf_results/` before further cleanup. Derived
from the old `processed_spectra_pkl/{chromatica,freya,wilhelm}_processed_spectrum.pkl`
lineage. The dynamic-spectrum DSA npz products for **all 12 bursts** are on
CANFAR at `$ARC/FLITS/scintillation/data/*.npz`; the staging recipe is to `vcp`
them to `$COD/results/dsa_scint_npz/`, then `rsync` to
`~/Data/Faber2026/dsa110/scintillation/data/` (§7a).
...
- **Pre-staged on h17:** `$COD/chime_singlebeam/` — singlebeam `.h5` for IDs
  210456524, 215063905, 224263996, 252069198, 253635173, 274819243, 278720455,
  311723353, 318353610, 354049284, 356959136, and 362593221. **[verified]**
  The July 2026 upchannelization pass used these local h17 voltages; no arc
  refetch was needed.
...
at-risk storage, not an authority. Notable subdirs **[verified]**:
...
**Regenerate CHIME targets if needed** (voltages already on h17, §4):
```bash
ssh h17
docker run --rm \
  -v /data:/data \
  ...
  python3 $COD/scripts/upchannelize_chime.py <nick> --no-time-shift --out $COD/upchan_codetections
# isha, hamilton, johndoeII: add --run-unresolvable for lower-confidence upper-bound/single-block products.
# outputs -> $COD/upchan_codetections/<nick>_chime_upchan.npy (+ _freq.npy + time0 metadata)
```
...
## 7a. CANFAR / arc — the authoritative remote copy (verified 2026-07-07)
...
## 7b. What the analysis notebooks actually used (per-burst, per-band)
