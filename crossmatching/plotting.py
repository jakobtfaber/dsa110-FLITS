...
def plot_toa_analysis(
    ...
    # Label with TNS designations (not internal nicknames), matching the manuscript.
    from scattering.scat_analysis.burst_metadata import load_tns_name

    for nickname, burst in results.items():
        try:
            nicknames.append(load_tns_name(nickname))
        except Exception:
    ...
    fig, (ax1, ax2) = plt.subplots(
        1, 2, figsize=figsize, gridspec_kw={"width_ratios": [2, 1]}, constrained_layout=True
    )
    ...
    ax1.axhspan(
        ...
        color="green",
        alpha=0.1,
        label=rf"Median Uncertainty ($\pm${med_err:.1f}ms)",
        zorder=0,
    )

    ax1.set_xticks(x_pos)
    ax1.set_xticklabels(nicknames, rotation=40, ha="right", fontsize=8.5)
    ...
    cbar.set_label("DM [pc cm$^{-3}$]", rotation=270, labelpad=15)
    ...
    ax2.set_xlabel("DM [pc cm$^{-3}$]", fontsize=12)
...
def main():
    ...
    plot_toa_analysis(
        results, output_path=output_dir / "toa_crossmatch_analysis_premium.png", show=False
    )

    # Generate the new systematics matrix
    plot_systematics_matrix(
        results, output_path=output_dir / "systematics_check_matrix.png", show=False
    )

    # Also save as PDF (LaTeX build) and SVG (vector source) for publication quality
    for ext in ("pdf", "svg"):
        plot_toa_analysis(
            results, output_path=output_dir / f"toa_crossmatch_analysis_premium.{ext}", show=False
        )
        plot_systematics_matrix(
            results, output_path=output_dir / f"systematics_check_matrix.{ext}", show=False
        )
