...
class FRBModel:
    ...
    def log_prior(self, params, model_type='model0'):
        """
    ...
    def log_posterior(self, params, model_type='model0'):
        """
...
def run_mcmc(model, initial_params, nwalkers=None, nsteps=200, model_type='model0'):
    ndim = len(initial_params)
    if nwalkers is None:
    ...
    idx = 4
    if model_type == 'model1':
        prior_bounds['zeta'] = (0, 1)
    elif model_type == 'model2':
        prior_bounds['tau_1GHz'] = (0, 1)
        prior_bounds['alpha'] = (1, 6)
    elif model_type == 'model3':
        prior_bounds['zeta'] = (0, 1)
        prior_bounds['tau_1GHz'] = (0, 1)
        prior_bounds['alpha'] = (1, 6)
...
def fit_models(model, init_params):
    """
    ...
    sampler0 = run_mcmc(model, initial_params0, model_type='model0')
    ...
    sampler1 = run_mcmc(model, initial_params1, model_type='model1')
    ...
    sampler2 = run_mcmc(model, initial_params2, model_type='model2')
    ...
    sampler3 = run_mcmc(model, initial_params3, model_type='model3')
