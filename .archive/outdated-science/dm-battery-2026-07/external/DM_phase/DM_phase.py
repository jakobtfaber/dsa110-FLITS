...
def _dm_calculation(waterfall, power_spectra, dpower_spectra, low_idx, up_idx,
    ...
    # VENDOR PATCH (numpy>=2 compat, see external/README.md): Start/Stop are
    # shape-(1,) arrays from argwhere-indexing; numpy 2.x removed their implicit
    # scalar conversion. No algorithm change.
    plot_range = np.arange(int(np.ravel(Start)[0]), int(np.ravel(Stop)[0]))
