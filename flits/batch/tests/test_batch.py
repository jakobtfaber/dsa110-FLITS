...
from ..analysis_logic import (
    ...
    build_comparison_df_from_joint_fits,
    check_tau_deltanu_consistency,
)
from ..config_generator import BurstInfo, ConfigGenerator
from ..joint_analysis import ConsistencyResult, JointAnalysis
...
class TestResultsDatabase:
    ...
    def test_get_comparison_table(
        self, temp_db, sample_scattering_result, sample_scintillation_result
    ):
...
class TestIntegration:
    ...
    def test_full_workflow(self, tmp_path):
        ...
        np.save(
            tmp_path / "data" / "chime" / "test_chime_I_500_0000_1000b_cntr_bpc.npy",
            np.zeros((64, 100)),
        )
        np.save(
            tmp_path / "data" / "dsa" / "test_dsa_I_500_0000_1000b_cntr_bpc.npy",
            np.zeros((64, 100)),
        )
...
# =============================================================================
# P1: joint-fit α  +  two-screen τ–Δν wiring
# =============================================================================


class TestP1JointScintWiring:
    """Real joint α_τ replaces the per-telescope-average placeholder, and the
    τ–Δν consistency check distinguishes a resolved near screen from the far
    pulse-broadening screen instead of bluntly calling them 'inconsistent'."""

    def test_joint_alpha_used_directly_not_averaged(self):
        # The joint fit gives ONE shared α per sightline. Carried on both
        # telescope rows it must be used as-is, not averaged (which fabricates
        # a √2 error shrink by treating two copies of one measurement as
        # independent).
        df = pd.DataFrame(
            [
                {
                    ...
                    "alpha_err": 0.05,
                    "alpha_joint": 2.71,
                    "alpha_joint_err": 0.05,
                    "delta_nu_dc": np.nan,
                    "delta_nu_dc_err": np.nan,
                },
                {
                    "burst_name": "wilhelm",
                    "telescope": "dsa",
        ...
        res = analyze_frequency_scaling(df)
        w = next(r for r in res if r.burst_name == "wilhelm")
        assert w.alpha_tau == pytest.approx(2.71)
        assert w.alpha_tau_err == pytest.approx(0.05)  # NOT 0.05/√2

    def test_resolved_dnu_far_tau_flagged_different_screens(self):
        # wilhelm: the fitted pulse-broadening τ is ~100× the τ implied by the
        # resolved scintillation Δν_d → different screens, not "inconsistent".
        df = pd.DataFrame(
            [
                {
                    "burst_name": "wilhelm",
        ...
        r = check_tau_deltanu_consistency(df)[0]
        assert r.implied_tau_from_dnu_ms is not None
        assert r.implied_tau_from_dnu_ms < r.tau_at_scint_freq_ms
        assert r.screen_verdict == "different_screens"
        assert not r.is_consistent
        assert "screen" in r.interpretation.lower()

    def test_same_screen_product_in_range_consistent(self):
        # A genuine single near screen: τ_at(1.4 GHz)=0.115 ms and Δν_d=1.6 kHz
        # satisfy 2π·τ·Δν≈C1, i.e. product = τ[s]·Δν[Hz] ≈ 0.18 ∈ [0.1, 2.0].
        df = pd.DataFrame(
            [
                {
                    "burst_name": "single",
                    ...
                    "alpha_err": 0.2,
                    "delta_nu_dc": 1.6e-3,
                    "delta_nu_dc_err": 1.6e-4,
                },
            ]
        )
        r = check_tau_deltanu_consistency(df)[0]
        assert r.tau_delta_nu_product == pytest.approx(0.18, abs=0.05)
        assert r.screen_verdict == "same_screen"
        assert r.is_consistent

    def test_empty_comparison_df_does_not_crash(self):
        # build_comparison_df_from_joint_fits on an empty dir yields a column-less
        # frame; the scaling analysis must no-op, not KeyError on groupby.
        assert analyze_frequency_scaling(pd.DataFrame()) == []

    def test_build_comparison_df_from_joint_fits(self, tmp_path):
        jdir = tmp_path / "joint_json"
        jdir.mkdir()
        (jdir / "wilhelm_joint_fit.json").write_text(
            json.dumps(
                {
                    "burst": "wilhelm",
                    "tau_1ghz": {"median": 0.26, "err_plus": 0.004, "err_minus": 0.004},
                    "alpha": {"median": 2.71, "err_plus": 0.05, "err_minus": 0.05},
                    "alpha_bounds": [1.0, 6.0],
                }
            )
        )
        df = build_comparison_df_from_joint_fits(jdir)
        assert set(df["telescope"]) == {"chime", "dsa"}
        assert (df["burst_name"] == "wilhelm").all()
        assert df["tau_1ghz"].iloc[0] == pytest.approx(0.26)
        assert df["alpha_joint"].iloc[0] == pytest.approx(2.71)
